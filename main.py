...
import traceback
# --- LOGGING ---
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(me
# --- SHRIFTNI AVTOMATIK YUKLASH (KOREYS VA RUS UCHUN) ---
FONT_PATH = "NanumGothic.ttf"
async def download_font():
if not os.path.exists(FONT_PATH):
try:
url = "https://github.com/google/fonts/raw/main/ofl/nanumgothic/NanumGothic-Regul
...
r = _req.get("https://github.com/google/fonts/raw/main/ofl/nanumgothic/NanumGothic-Re
with open(FONT_PATH, "wb") as f:
f.write(r.content)
except Exception as e:
...
try:
pdfmetrics.registerFont(TTFont('DejaVu', '/usr/share/fonts/truetype/dejavu/DejaVuSans
pdfmetrics.registerFont(TTFont('DejaVu-Bold', '/usr/share/fonts/truetype/dejavu/DejaV
FONT_NAME = 'DejaVu'
...
logging.warning(" Maxsus shrift topilmadi, Helvetica ishlatiladi")
...
return re.sub(r'[^\w\s\d\.\:\-\/\[\]\(\)\|\!\?\,\u0400-\u04FF\uAC00-\uD7AF\u1100-\u11FF]+
# --- SOZLAMALAR ---
...
async with self.pool.acquire() as conn:
await conn.execute("UPDATE users SET status = $1 WHERE user_id = $2", status, use
...
async with self.pool.acquire() as conn:
await conn.execute("""
INSERT INTO broadcast_history (message_text, sent_count, failed_count, VALUES ($1, $2, $3, $4)
...
students = await conn.fetch("SELECT * FROM group_students WHERE group_id = $1
group_students[g['id']] = [{'name': s['student_name'], 'phone': s['student_ph
logging.info(f" RAM ga yuklandi: {len(user_ids)} foydalanuvchi, {len(daily_attendan
...
WEEKDAY_ORDER = {
'Dushanba': 0, 'Seshanba': 1, 'Chorshanba': 2, 'Payshanba': 3, 'Juma': 4, 'Shanba': 5, 'Y
}
LESSON_TYPES = {
'uz':['IT', 'Koreys tili'],
...
"Clear": {
"uz": " "ru": " "kr": " Bugun havo ochiq. Sayr qilish uchun ajoyib kun!",
Сегодня ясно. Отличный день для прогулки!",
오늘은 맑은 날씨입니다. 산책하기 좋은 날이에요!"
},
"Clouds": {
"uz": " "ru": " "kr": " Bugun havo bulutli. Salqin havo bilan ish kuningiz samarali o'tsin!",
Сегодня облачно. Пусть прохладная погода сделает ваш рабочий день продуктив
오늘은 흐린 날씨입니다. 시원한 날씨와 함께 즐거운 하루 되세요!"
},
"Rain": {
"uz": " "ru": " "kr": " Bugun yomg'ir yog'moqda. Soyabon olishni unutmang!",
Сегодня идет дождь. Не забудьте взять зонтик!",
오늘은 비가 옵니다. 우산 챙기는 것 잊지마세요!"
},
"Thunderstorm": {
"uz": " Momaqaldiroq bo'lmoqda. Ehtiyot bo'ling!",
"ru": " Гроза. Будьте осторожны!",
"kr": " 천둥번개가 칩니다. 조심하세요!"
},
"Snow": {
"uz": " "ru": " "kr": " Qor yog'moqda. Issiq kiyining!",
Идет снег. Одевайтесь теплее!",
눈이 옵니다. 따뜻하게 입으세요!"
},
"Mist": {
"uz": " "ru": " "kr": " Tuman tushgan. Haydovchilar ehtiyot bo'ling!",
Туман. Водители, будьте осторожны!",
안개가 끼었습니다. 운전자분들 조심하세요!"
},
"Fog": {
"uz": " "ru": " "kr": " Tuman tushgan. Haydovchilar ehtiyot bo'ling!",
Туман. Водители, будьте осторожны!",
...
"Haze": {
"uz": " Havo tumanli. Ehtiyot bo'ling!",
"ru": " Дымка. Будьте осторожны!",
"kr": " 연무가 끼었습니다. 조심하세요!"
}
}
TRANSLATIONS = {
'uz': {
'welcome': " HANCOM ACADEMYning o'qituvchilar uchun davomat botiga hush kelibsiz, {
...
'already_attended': " Siz bugun {branch} hududida allaqachon davomatdan o'tgansiz!"
'not_in_area': " Siz belgilangan ta'lim muassasalari hududida emassiz!",
'daily_reminder': " Eslatma! Bugun hali davomat qilmagansiz. Ish kuningizni boshlas
...
'lesson_started_attended': " Dars boshlandi va siz muvaffaqiyatli davomatni amalga
'lesson_started_not_attended': " Sizning darsingiz boshlandi, lekin hali davomat qi
...
'ru': {
'welcome': " 'ask_name': " 'ask_specialty': " Добро пожаловать в бот для отметок HANCOM ACADEMY для учителей, Пожалуйста, введите ваше имя и фамилию:\n\nНапример: Ali Karimov",
...
'already_attended': " Вы уже отмечались сегодня в филиале {branch}!",
'not_in_area': " Вы не находитесь в зоне учебных заведений!",
'daily_reminder': " Напоминание! Вы еще не отметились сегодня. Подтвердите свое при
...
'lesson_started_not_attended': " Ваш урок начался, но вы еще не отметились!\n\n В
...
'already_attended': " 오늘 이미 {branch} 지점에서 출석 체크하셨습니다!",
...
'reminder': " 알림!\n\n오늘 {time}에 {branch} 지점에서 수업이 있습니다.\n출석 체크를 잊지 'lesson_started_attended': " 수업이 시작되었고 출석이 확인되었습니다!\n\n수업에 집중하세요.\n학생들
'lesson_started_not_attended': " 수업이 시작되었지만 아직 출석 체크하지 않으셨습니다!\n\n 마세요!
{bra
...
order = {'Dushanba': 0, 'Seshanba': 1, 'Chorshanba': 2, 'Payshanba': 3, 'Juma': 4, return dict(sorted(days_dict.items(), key=lambda x: order.get(x[0], 7)))
'Shanb
...
title = Paragraph(f"Davomat Hisoboti: {report_date.strftime('%d.%m.%Y')} ({report_weekday
elements.append(title)
elements.append(Spacer(1, 15))
data = [['№', 'Davomat', 'Dars', 'O\'qituvchi', 'Mutaxassislik', 'Filial', 'Holat', 'Kech
for i, att in enumerate(sorted(check_ins, key=lambda x: x[3] if x[3] != "00:00:00" uid = att[0]
branch = att[1]
att_time = att[3]
...
data.append([str(i), att_time_disp, lesson_time, teacher_name, specialty, branch, sta
table = Table(data, colWidths=[0.4*inch, 0.9*inch, 0.9*inch, 1.8*inch, 1.0*inch, 1.5*inch
style_commands = [
...
status = row_data[6]
if status == "Kechikkan":
style_commands.append(('BACKGROUND', (6, row_idx), (6, row_idx), colors.HexColor(
...
title_text = f"<b>{name.upper()}</b> | {specialty} | {clean_pdf_text(TRANSLATIONS[lang]['
elements.append(Paragraph(title_text, title_style))
sched_ids = user_schedules.get(user_id, [])
if not sched_ids:
elements.append(Paragraph(clean_pdf_text(TRANSLATIONS[lang]['no_schedules']), normal_
else:
...
ParagraphStyle('B', fontName=title_font, fontSize=16, textCo
...
uz_days = ["Dushanba", "Seshanba", "Chorshanba", "Payshanba", "Juma", "Shanba", "
...
('GRID', (0, 0), (-1, -1), 0.5, colors.grey),
('FONTSIZE', (0, 0), (-1, -1), 13),
('ROWBACKGROUNDS', (0, 1), (-1, -1), [colors.white, colors.HexColor('#F5F
('TOPPADDING', (0, 0), (-1, -1), 10),
('BOTTOMPADDING', (0, 0), (-1, -1), 10),
]))
elements.append(t)
...
footer_text = f"{clean_pdf_text(TRANSLATIONS[lang]['pdf_created'])}: {datetime.now(UZB_TZ
elements.append(Paragraph(footer_text, ParagraphStyle('F', fontName=FONT_NAME, fontSize=1
doc.build(elements)
pdf_buffer.seek(0)
return pdf_buffer
...
f" Bugungi davomatlar: {len([k for k in daily_attendance_log if k[2] == f" Guruhlar: {len(groups)} ta"
now_uz
)
...
await message.answer("Tilni tanlang / Выберите язык / 언어를 선택하세요:", reply_markup=builde
...
lang=lang_display)
builder = InlineKeyboardBuilder()
builder.row(InlineKeyboardButton(text=get_text(user_id, 'edit_name'), callback_data="edit
builder.row(InlineKeyboardButton(text=get_text(user_id, 'edit_my_specialty'), callback_da
builder.row(InlineKeyboardButton(text=get_text(user_id, 'back_to_menu'), callback_data="b
await message.answer(
profile_text,
...
builder.row(InlineKeyboardButton(text=" Koreys tili", callback_data="save_spec_Koreys t
builder.row(InlineKeyboardButton(text=" Ofis xodimi", callback_data="save_spec_Ofis xod
builder.row(InlineKeyboardButton(text=get_text(uid, 'back_btn'), callback_data="back_to_p
...
Ism juda qisqa. Iltimos, qaytadan kiriting:")
...
@dp.message(F.text.in_({'\U0001F4CB Dars jadvalim (PDF)', '\U0001F4CB Мое расписание (PDF)',
...
@dp.message(F.text.in_({'\U0001F4CA Mening statistikam', '\U0001F4CA Моя статистика', '\U0001
...
weekdays =["Dushanba", "Seshanba", "Chorshanba", "Payshanba", "Juma", "Shanba", "Yaks
current_month_text = "(joriy oy)"
elif lang == 'ru':
month_names = month_names_ru
weekdays =["Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", current_month_text = "(текущий месяц)"
"Воскр
else:
...
lang = user_languages.get(user_id, 'uz')
if lang == 'uz':
no_data_msg = " elif lang == 'ru':
no_data_msg = " Bu hafta hali davomat yo'q"
На этой неделе еще нет отметок"
else:
...
user_warning = (
" **DIQQAT: SOXTA DAVOMATGA URINISH!**\n\n"
...
already_attended = any(k[0] == user_id and k[1] == found_branch and k[2] == today_dat
if already_attended:
await message.answer(get_text(user_id, 'already_attended', branch=found_branch),
return
...
weather_message = format_weather_message(weather_data, user_languages.get(user_id, 'u
...
return emoji_map.get(weather_condition, " ")
...
recommendation_title = "Tavsiya" if lang == 'uz' else "Рекомендация" if lang == 'ru' else
time_text = "Vaqt" if lang == 'uz' else "Время" if lang == 'ru' else "시간"
message = f"""
{emoji} Ob-havo ma'lumoti
Joy: {city}
{temp_text}: {temp:.1f}°C ({feels_text}: {feels_like:.1f}°C)
...
builder = InlineKeyboardBuilder()
month_names = {
'uz': ["Yanvar", "Fevral", "Mart", "Aprel", "May", "Iyun", "Iyul", "Avgust", "Sentabr
'ru': ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", 'kr': ["1월", "2월", "3월", "4월", "5월", "6월", "7월", "8월", "9월", "10월", "11월", "12월"
"Сентя
}
wd_names = {
...
builder.row(
InlineKeyboardButton(text=" Vizual Jadval (Haftalik)", callback_data="admin_vis
)
builder.row(
InlineKeyboardButton(text=" Guruh shakllantirish", callback_data="admin_create_
)
builder.row(
...
InlineKeyboardButton(text=" Dars jadvallari", callback_data="admin_schedules_ma
)
builder.row(
InlineKeyboardButton(text=" Oylik hisobot (Excel)", callback_data="admin_excel_
...
await callback.message.edit_text("Qaysi filialga guruh qo'shmoqchisiz?", reply_markup=bui
...
InlineKeyboardButton(text=" Koreys tili", callback_data="grp_type_Koreys ti
...
builder.row(InlineKeyboardButton(text=" Keyingisi", callback_data="grp_days_next"))
await message.edit_text("Dars kunlarini tanlang (bir nechta tanlashingiz mumkin):", reply
...
await message.answer(" Noto'g'ri format! Iltimos, HH:MM formatida kiriting (masalan
return
...
await message.answer(
f" f"Davom etamizmi?",
O'quvchi qo'shildi! Jami: {len(students)} ta.\n\n"
reply_markup=builder.as_markup()
)
...
INSERT INTO groups (group_name, branch, lesson_type, teacher_id, days_data, t
...
time_slots = ['08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16
...
ha='center', va='center', fontsize=8, fontweight='bold', zor
except: continue
...
plt.title(f" plt.xlim(0, len(days))
plt.ylim(0, len(time_slots))
{branch_name} - Haftalik Bandlik Jadvali", fontsize=16, pad=20, fontweight
from matplotlib.lines import Line2D
legend_elements = [
Line2D([0], [0], marker='s', color='w', label='IT Bo\'limi', markerfacecolor='#E3F2FD
Line2D([0], [0], marker='s', color='w', label='Koreys tili', markerfacecolor='#E8F5E9
Line2D([0], [0], marker='s', color='w', label='Ofis xodimi', markerfacecolor='#FFF3E0
]
ax.legend(handles=legend_elements, loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol=
img_buf = io.BytesIO()
...
if not found:
await callback.message.answer(f" {branch_name} filialida hali darslar belgilanm
else:
...
if not branches:
await callback.message.answer(" await state.clear()
return
Bu o'qituvchiga dars jadvali biriktirilmagan!")
...
await message.answer(f" Filial: {current_branch}\n\nUshbu filialdagi o'quvchilar await state.set_state(SalaryCalc.entering_students)
sonini
...
builder.adjust(3)
await message.answer("Imtixon natijasini tanlang (%):", reply_markup=builder.as_marku
...
data = await state.get_data()
if data['specialty'] == "IT":
await callback.message.edit_text("Ushbu filial uchun jarima FOIZINI kiriting (masalan
await state.set_state(SalaryCalc.entering_penalty_it_percent)
else:
await callback.message.edit_text("Ushbu filial uchun jarima SUMMASINI kiriting await state.set_state(SalaryCalc.entering_penalty_kr_sum)
await callback.answer()
(so'md
...
net = total_gross - tax
excel_file = await create_multi_branch_excel(data['teacher_name'], data['specialty'], res
...
await message.answer_document(
types.BufferedInputFile(excel_file.read(), filename=f"Oylik_{data['teacher_name']}_{d
caption=caption
)
await state.clear()
builder = InlineKeyboardBuilder()
builder.row(InlineKeyboardButton(text=" Yangi hisoblash", callback_data="admin_salary_c
...
headers = ['Filial', 'O\'quvchilar', 'Darslar', 'Imtixon %', 'Jarima', 'Tushum', 'Hisobla
ws.append(headers)
for cell in ws[1]:
cell.fill = header_fill
...
ws.cell(row=ws.max_row, column=7).fill = PatternFill(start_color="C6EFCE"
...
7: "Iyul", 8: "Avgust", 9: "Sentabr", 10: "Oktabr", 11: "Noyabr", 12: "Dekab
temp_date = start_date
while temp_date <= now:
month_name = months_uz[temp_date.month]
btn_text = f" {month_name} {temp_date.year}"
builder.row(InlineKeyboardButton(text=btn_text, callback_data=f"gen_month_{temp_date.
...
builder.row(InlineKeyboardButton(text=" Ortga", callback_data="admin_back"))
await callback.message.edit_text("Qaysi oy uchun hisobot kerak?", reply_markup=builder.as
await callback.answer()
...
styles = getSampleStyleSheet()
months_uz = {1: "Yanvar", 2: "Fevral", 3: "Mart", 4: "Aprel", 5: "May", 6: "Iyun",
7: "Iyul", 8: "Avgust", 9: "Sentabr", 10: "Oktabr", 11: "Noyabr", 12: title_style = ParagraphStyle(
'Title',
parent=styles['Heading1'],
fontSize=20,
alignment=1,
...
elements.append(Paragraph(f"{months_uz[month]} {year} - Oylik Davomat Hisoboti", title_st
...
user_header_fill = PatternFill(start_color="D9D9D9", fill_type="solid") # Kulrang o'qituv
branch_header_fill = PatternFill(start_color="FDE9D9", fill_type="solid") # Och jigarrang
table_header_fill = PatternFill(start_color="2E86AB", fill_type="solid") # Ko'k jadval he
...
ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, user_title = ws.cell(row=current_row, column=1)
user_title.value = f" {user_names.get(uid, '').upper()} {months_uz[month]} OYLI
...
teacher_branches = sorted(list(set([s['branch'] for sid, s in schedules.items() i
for branch in teacher_branches:
# 4. Filial sarlavhasi (Och jigarrang)
ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, en
...
current_row += 1
# 5. Jadval headeri
headers = ['№', 'Sana', 'Hafta kuni', 'O\'qituvchi', 'Filial', 'Dars vaqti',
...
for sid, sdata in schedules.items():
if sdata['user_id'] == uid and sdata['branch'] == branch and weekday
sch_time = sdata['days'][weekday]
att = next((a for a in daily_attendance_log if a[0] == uid and a[
if att:
...
att_time = "—"
late_m = "—"
row_vals = [num, target_date, weekday, user_names.get(uid), branc
...
cell.fill = PatternFill(start_color="FFCCCC", fill_ty
num += 1
current_row += 1
...
builder.row(InlineKeyboardButton(text=btn_text, callback_data=f"get_excel_{temp_date.
if temp_date.month == 12:
temp_date = datetime(temp_date.year + 1, 1, 1, tzinfo=UZB_TZ)
else:
temp_date = datetime(temp_date.year, temp_date.month + 1, 1, tzinfo=UZB_TZ)
builder.row(InlineKeyboardButton(text=" Ortga", callback_data="admin_back"))
await callback.message.edit_text("Qaysi oy uchun Excel hisobot kerak?", reply_markup=buil
await callback.answer()
...
elements.append(Paragraph(f"Hisobot yaratilgan sana: {datetime.now(UZB_TZ).strftime('%d.%
elements.append(Spacer(1, 20))
now_uzb = datetime.now(UZB_TZ)
...
monthly_attendances = len([k for k in daily_attendance_log if k[2].startswith(current_mon
...
elements.append(Paragraph(f"Hisobot yaratilgan sana: {now_uzb.strftime('%d.%m.%Y %H:%M')}
elements.append(Spacer(1, 20))
...
for i, (branch, users) in enumerate(sorted(monthly_stats.items(), key=lambda x: sum(x[1].
...
filename = f"filiallar_reytingi_{datetime.now(UZB_TZ).strftime('%Y%m%d_%H%M')}.pd
caption = " Filiallar reytingi"
elif report_type == "teachers":
pdf_buffer = await create_teachers_stats_pdf()
filename = f"oqituvchilar_reytingi_{datetime.now(UZB_TZ).strftime('%Y%m%d_%H%M')}
...
builder = InlineKeyboardBuilder()
builder.row(
InlineKeyboardButton(text=" Umumiy statistika", callback_data="admin_stats_gene
InlineKeyboardButton(text=" Filiallar reytingi", callback_data="admin_stats_bra
)
builder.row(
InlineKeyboardButton(text=" O'qituvchilar reytingi", callback_data="admin_stats
...
total_attendances = len(daily_attendance_log)
today_attendances = len([k for k in daily_attendance_log if k[2] == today])
monthly_attendances = len([k for k in daily_attendance_log if k[2].startswith(current
it_teachers = len([uid for uid in user_ids if user_specialty.get(uid) == 'IT'])
korean_teachers = len([uid for uid in user_ids if user_specialty.get(uid) == 'Koreys
office_workers = len([uid for uid in user_ids if user_specialty.get(uid) == 'Ofis xod
branch_stats = defaultdict(int)
for (uid, branch, date, time) in daily_attendance_log:
branch_stats[branch] += 1
top_branch = max(branch_stats.items(), key=lambda x: x[1]) if branch_stats else ("Yo'
teacher_stats = defaultdict(int)
for (uid, branch, date, time) in daily_attendance_log:
teacher_stats[uid] += 1
top_teacher_id = max(teacher_stats.items(), key=lambda x: x[1]) if teacher_stats else
top_teacher_name = user_names.get(top_teacher_id[0], "Noma'lum") if top_teacher_id[0]
top_teacher_specialty = user_specialty.get(top_teacher_id[0], '')
top_teacher_display = f"{top_teacher_name}[{top_teacher_specialty}]" if top_teacher_s
text = f"""
Umumiy statistika
Foydalanuvchilar:
• Jami: {total_users}
...
for i, (branch, count) in enumerate(sorted_branches, 1):
medal = " " if i == 1 else " " if i == 2 else " text += f"{medal} {branch}: {count} ta davomat\n"
" if i == 3 else f"{i}."
builder = InlineKeyboardBuilder()
builder.row(InlineKeyboardButton(text=" Ortga", callback_data="admin_stats_main"))
await callback.message.edit_text(
...
await callback.message.edit_text(" await callback.answer()
Filiallar reytingini olishda xatolik yuz berdi")
...
await callback.message.edit_text(" await callback.answer()
O'qituvchilar reytingini olishda xatolik yuz ber
...
await callback.message.edit_text(" await callback.answer()
return
Shu oy uchun davomat ma'lumotlari yo'q.")
...
builder = InlineKeyboardBuilder()
builder.row(
InlineKeyboardButton(text=" Foydalanuvchilar ro'yxati", callback_data="admin_us
)
builder.row(
...
Holat: {" Faol" if status == 'active' else " Til: {lang}
Bloklangan"}
Statistika:
...
builder = InlineKeyboardBuilder()
if status == 'active':
builder.row(InlineKeyboardButton(text=" Bloklash", callback_data=f"admin_user_block
else:
builder.row(InlineKeyboardButton(text=" Faollashtirish", callback_data=f"admin_user
builder.row(
...
logging.error(f"admin_user_delete_confirm parse error: {callback.data}, error: await callback.answer("Noto'g'ri format!")
return
{e}")
try:
async with db.pool.acquire() as conn:
...
builder = InlineKeyboardBuilder()
builder.row(
InlineKeyboardButton(text=" Ha, o'chirish", callback_data=f"admin_user_delete_c
...
specialty_display = f" [{specialty}]" if specialty else ""
builder.row(
InlineKeyboardButton(text=f" {name}{specialty_display}", callback_data=f"ad
)
builder.row(InlineKeyboardButton(text=" Ortga", callback_data="admin_users_main"))
await callback.message.edit_text(
" Faol foydalanuvchilar ro'yxati:",
...
await callback.message.edit_text(" await callback.answer()
Faol foydalanuvchilar ro'yxatini olishda xatolik
...
await callback.message.edit_text(" await callback.answer()
Bloklangan foydalanuvchilar ro'yxatini olishda x
...
builder = InlineKeyboardBuilder()
builder.row(
InlineKeyboardButton(text=TRANSLATIONS[lang]['specialty_it'], callback_data="broadcas
InlineKeyboardButton(text=TRANSLATIONS[lang]['specialty_korean'], callback_data="broa
)
builder.row(
InlineKeyboardButton(text=" Ofis xodimi", callback_data="broadcast_spec_Ofis xodimi
InlineKeyboardButton(text=TRANSLATIONS[lang]['all_teachers'], callback_data="broadcas
)
builder.row(InlineKeyboardButton(text=" Ortga", callback_data="admin_back"))
...
await callback.message.edit_text(
" Xabar yuborish\n\nYubormoqchi bo'lgan xabaringizni kiriting (matn, rasm, hujj
)
await callback.answer()
except Exception as e:
...
if specialty:
target_users =[uid for uid in user_ids if user_status.get(uid) != 'blocked' and u
...
await bot.send_photo(user_id, msg_data.photo[-1].file_id, caption=msg_dat
elif data['message_type'] == 'document':
await bot.send_document(user_id, msg_data.document.file_id, caption=msg_d
sent_count += 1
...
await db.save_broadcast(data.get('message_text', ''), sent_count, failed_count, speci
...
builder = InlineKeyboardBuilder()
builder.row(
InlineKeyboardButton(text=" O'qituvchiga jadval qo'shish", callback_data="admin
)
builder.row(
InlineKeyboardButton(text=" Faol dars jadvallari", callback_data="admin_active_
)
builder.row(
InlineKeyboardButton(text=" Dars jadvali PDF", callback_data="admin_schedules_p
)
builder.row(
InlineKeyboardButton(text=" Ortga", callback_data="admin_back")
)
...
filename = f"barcha_dars_jadvallari_{datetime.now(UZB_TZ).strftime('%Y%m%d_%H%M')}.pd
await callback.message.delete()
await callback.message.answer_document(
types.BufferedInputFile(pdf_buffer.getvalue(), filename=filename),
...
ParagraphStyle('Sub', parent=normal_style, alignment=1, fontSize=12, spaceAfter=20, f
))
...
teacher_block.append(Paragraph(f" Filial: {branch} ({l_type})", branch_info
...
footer_text = f"Hisobot yaratildi: {datetime.now(UZB_TZ).strftime('%d.%m.%Y %H:%M')} | Ha
elements.append(Paragraph(
footer_text,
ParagraphStyle('Footer', parent=normal_style, fontSize=8, alignment=2, textColor=colo
))
doc.build(elements)
pdf_buffer.seek(0)
return pdf_buffer
...
await callback.message.edit_text(" await callback.answer()
return
Hali dars jadvallari mavjud emas.")
...
days_times = ""
for day, time in sorted(schedule['days'].items(), key=lambda x: WEEKDAY_ORDER.get
days_times += f"• {day}: {time}\n"
text = f"{teacher_name}{specialty_display}\n {branch}\n {lesson_type}\n{days_
builder = InlineKeyboardBuilder()
builder.row(
InlineKeyboardButton(text=" O'zgartirish", callback_data=f"admin_edit_sched
InlineKeyboardButton(text=" O'chirish", callback_data=f"admin_delete_schedu
)
await callback.message.answer(
text,
reply_markup=builder.as_markup()
...
await callback.message.edit_text(" await callback.answer()
Dars jadvallarini olishda xatolik yuz berdi")
...
builder.row(
InlineKeyboardButton(text=location['name'], callback_data=f"edit_branch_{loca
)
...
builder.row(
InlineKeyboardButton(text=" Keyingisi", callback_data="edit_weekdays_next")
)
...
await message.answer(" return
Noto'g'ri format! Iltimos, HH:MM formatida kiriting (masalan
data = await state.get_data()
selected_days = data.get('edit_selected_days', {})
...
schedules[new_s_id] = {'user_id': teacher_id, 'branch': new_branch, 'lesson_type': user_schedules[teacher_id].append(new_s_id)
...
Yangi dars jadvalingiz (Landscape formatda)"
caption=" )
except Exception as e:
logging.error(f"Notify error: {e}")
await message.answer(" await state.clear()
Jadval yangilandi, eskisi o'chirildi va o'qituvchiga xabar yubor
...
specialty_display = f" [{specialty}]" if specialty else ""
builder.row(
InlineKeyboardButton(text=f" {name}{specialty_display}", callback_data=
)
...
for location in LOCATIONS:
builder.row(
InlineKeyboardButton(text=location['name'], callback_data=f"admin_branch_{loc
)
...
builder.row(
InlineKeyboardButton(text=" Keyingisi", callback_data="admin_weekdays_next")
)
...
builder = InlineKeyboardBuilder()
builder.row(
InlineKeyboardButton(text=" Yangi filial qo'shish", callback_data="admin_locati
InlineKeyboardButton(text=" Barcha filiallar", callback_data="admin_location_li
)
builder.row(
InlineKeyboardButton(text=" Ortga", callback_data="admin_back")
)
...
await callback.message.edit_text(" await callback.answer()
Filiallar ro'yxatini olishda xatolik yuz berdi")
...
await callback.message.edit_text(" await callback.answer()
Admin panelga qaytishda xatolik yuz berdi")
...
document=types.BufferedInputFile(pdf_buf.read(), filename=f"hisobot_{
...
msg = (f" ESLATMA\n\n"
f"Bugun soat {lesson_time} da {branch} filialida darsingiz boshlan
f" Iltimos, darsingizni davomatini qilishni unutmang!")
try:
await bot.send_message(user_id, msg)
logging.info(f"1-min reminder sent to {user_id} for {branch} at {less
except Exception as e:
logging.error(f"Failed to send reminder to {user_id}: {e}")
elif current_time == check_time:
attended = any(k[0] == user_id and k[1] == branch and k[2] == today_date
if not attended:
msg = (f" DIQQAT: DAVOMAT QILINMADI!\n\n"
f"Darsingiz soat {lesson_time} da {branch} filialida boshlanga
