...
async def grp_branch_selected(callback: types.CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="💻 IT", callback_data="grp_type_IT"),
        InlineKeyboardButton(text="🇰🇷 Koreys tili", callback_data="grp_type_Koreys tili")
    )
    await callback.message.edit_text("Dars turini tanlang:", reply_markup=builder.as_markup())
    await state.set_state(CreateGroup.selecting_type)
...
@dp.callback_query(CreateGroup.selecting_type, F.data.startswith("grp_type_"))
async def grp_type_selected(callback: types.CallbackQuery, state: FSMContext):
    await state.update_data(lesson_type=callback.data.replace("grp_type_", ""))
    builder = InlineKeyboardBuilder()
    for uid, name in user_names.items():
        if user_specialty.get(uid) not in (None, "Ofis xodimi"):
            builder.row(InlineKeyboardButton(text=str(name), callback_data=f"grp_tchr_{uid}"))
    await callback.message.edit_text("O'qituvchini tanlang:", reply_markup=builder.as_markup())
    await state.set_state(CreateGroup.selecting_teacher)
    await callback.answer()
...
async def grp_show_days(message: types.Message, selected: list):
    builder = InlineKeyboardBuilder()
    days = ["Dushanba", "Seshanba", "Chorshanba", "Payshanba", "Juma", "Shanba", "Yakshanba"]
    for day in days:
        text = f"✅ {day}" if day in selected else f"⬜ {day}"
        builder.row(InlineKeyboardButton(text=text, callback_data=f"grp_day_{day}"))
    builder.row(InlineKeyboardButton(text="➡️ Keyingisi", callback_data="grp_days_next"))
    await message.edit_text("Dars kunlarini tanlang:", reply_markup=builder.as_markup())
...
async def grp_time_entered(message: types.Message, state: FSMContext):
    if not re.match(r"^([01]?[0-9]|2[0-3]):[0-5][0-9]$", message.text.strip()):
        await message.answer("❌ Noto'g'ri format! HH:MM kiriting (masalan: 14:00):")
        return
    await state.update_data(time_text=message.text.strip())
    await message.answer("Guruh nomini kiriting (masalan: IT-A yoki Koreys-1):")
    await state.set_state(CreateGroup.entering_group_name)

@dp.message(CreateGroup.entering_group_name)
async def grp_name_entered(message: types.Message, state: FSMContext):
    await state.update_data(group_name=message.text.strip(), students=[])
    await message.answer("Oquvchilar qoshishni boshlaymiz.\n\n1-oquvchining ismini kiriting:")
    await state.set_state(CreateGroup.adding_student_name)
...
async def grp_student_phone(message: types.Message, state: FSMContext):
    ...
    students.append({"name": data["current_std_name"], "phone": message.text.strip()})
    await state.update_data(students=students)
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="➕ Yana o'quvchi", callback_data="grp_add_more"),
        InlineKeyboardButton(text="✅ Yakunlash", callback_data="grp_finish")
    )
    await message.answer(f"Qoshildi! Jami: {len(students)} ta. Davom etamizmi?", reply_markup=builder.as_markup())
    await state.set_state(CreateGroup.confirm_student)

@dp.callback_query(CreateGroup.confirm_student, F.data == "grp_add_more")
async def grp_more_students(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    await callback.message.edit_text(f"{len(data['students'])+1}-o'quvchi ismini kiriting:")
    await state.set_state(CreateGroup.adding_student_name)
    await callback.answer()
    ...
    try:
        async with db.pool.acquire() as conn:
            group_id = await conn.fetchval("""
                INSERT INTO groups (group_name, branch, lesson_type, teacher_id, days_data, time_text)
                VALUES ($1, $2, $3, $4, $5::jsonb, $6) RETURNING id
            """, data["group_name"], data["branch"], data["lesson_type"], data["teacher_id"],
               json.dumps(data["selected_days"]), data["time_text"])
            for std in data["students"]:
                await conn.execute(
        ...
        groups[group_id] = {
            "group_name": data["group_name"], "branch": data["branch"],
            "lesson_type": data["lesson_type"], "teacher_id": data["teacher_id"],
            "days": data["selected_days"], "time": data["time_text"]
        }
        group_students[group_id] = data["students"]
        days_str = ", ".join(data["selected_days"])
        try:
            await bot.send_message(data["teacher_id"],
                f"🆕 *Yangi guruh biriktirildi!*\n\n"
                f"📦 Guruh: {data['group_name']}\n🏢 Filial: {data['branch']}\n"
                f"📚 Fan: {data['lesson_type']}\n⏰ Vaqt: {data['time_text']}\n"
                f"📅 Kunlar: {days_str}\n👥 O'quvchilar: {len(data['students'])} ta",
                parse_mode="Markdown")
        except Exception as e:
            logging.error(f"Teacher notify error: {e}")
        await callback.message.edit_text(
            f"✅ *{data['group_name']}* guruhi yaratildi!\n\n"
            f"👥 O'quvchilar: {len(data['students'])} ta",
            parse_mode="Markdown"
        )
    except Exception as e:
        logging.error(f"grp_final_save error: {e}")
...
async def admin_active_groups(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not all_grps:
            builder = InlineKeyboardBuilder()
            builder.row(InlineKeyboardButton(text="🔙 Ortga", callback_data="admin_back"))
            await callback.message.edit_text("📭 Hozircha faol guruhlar yo'q.", reply_markup=builder.as_markup())
            await callback.answer()
            return
        builder = InlineKeyboardBuilder()
        for grp in all_grps:
            teacher_name = user_names.get(grp["teacher_id"], "Noma'lum")
            btn_text = f"👥 {grp['group_name']} | {grp['branch']} | {teacher_name}"
            builder.row(InlineKeyboardButton(text=btn_text, callback_data=f"grp_view_{grp['id']}"))
        builder.row(InlineKeyboardButton(text="🔙 Ortga", callback_data="admin_back"))
        await callback.message.edit_text(
...
async def grp_view_detail(callback: types.CallbackQuery, state: FSMContext):
    if not check_admin(callback.message.chat.id):
        ...
        return
    try:
        group_id = int(callback.data.replace("grp_view_", ""))
        async with db.pool.acquire() as conn:
        ...
        if not grp:
            ...
            return
        teacher_name = user_names.get(grp["teacher_id"], "Noma'lum")
        days_list = grp["days_data"] if isinstance(grp["days_data"], list) else []
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="✏️ O'quvchilarni tahrirlash", callback_data=f"grp_edit_{group_id}"))
        builder.row(
...
@dp.callback_query(F.data.startswith("grp_edit_"))
...
async def grp_std_edit_options(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not std:
            ...
            return
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="✏️ Ismini o'zgartirish", callback_data=f"grp_std_rename_{std_id}_{group_id}"))
...
async def grp_std_rename_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("✏️ Yangi ismni kiriting:")
    await callback.answer()
...
async def grp_std_save_name(message: types.Message, state: FSMContext):
    ...
    if data.get("add_std_step") == "name":
        ...
        await state.update_data(add_std_name=new_name, add_std_step="phone")
        await state.set_state(EditGroupStudents.entering_new_phone)
        await message.answer(f"📱 *{new_name}* ning telefon raqamini kiriting:", parse_mode="Markdown")
        return
    std_id, group_id = data["edit_std_id"], data["edit_group_id"]
    try:
        async with db.pool.acquire() as conn:
            await conn.execute("UPDATE group_students SET student_name=$1 WHERE id=$2", new_name, std_id)
        await message.answer(f"✅ Ism yangilandi: *{new_name}*", parse_mode="Markdown")
    except Exception as e:
        await message.answer(f"❌ Xatolik: {e}")
    await state.clear()
    builder = InlineKeyboardBuilder()
...
async def grp_std_save_phone(message: types.Message, state: FSMContext):
    ...
    if data.get("add_std_step") == "phone":
        ...
        try:
            async with db.pool.acquire() as conn:
                await conn.execute(
                    "INSERT INTO group_students (group_id, student_name, student_phone) VALUES ($1,$2,$3)",
                    group_id, new_name, new_phone
                )
            await message.answer(f"✅ *{new_name}* qo'shildi!", parse_mode="Markdown")
        except Exception as e:
            await message.answer(f"❌ Xatolik: {e}")
        await state.clear()
        builder = InlineKeyboardBuilder()
    ...
    try:
        async with db.pool.acquire() as conn:
            await conn.execute("UPDATE group_students SET student_phone=$1 WHERE id=$2", new_phone, std_id)
        await message.answer(f"✅ Raqam yangilandi: *{new_phone}*", parse_mode="Markdown")
    except Exception as e:
        await message.answer(f"❌ Xatolik: {e}")
    await state.clear()
    builder = InlineKeyboardBuilder()
...
async def grp_std_delete(callback: types.CallbackQuery, state: FSMContext):
    ...
    for idx, std in enumerate(students, 1):
        text += f"{idx}. {std['student_name']} — {std['student_phone']}\n"
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="✏️ Tahrirlash", callback_data=f"grp_edit_{group_id}"))
    builder.row(
        InlineKeyboardButton(text="📥 Excel yuklab olish", callback_data=f"grp_excel_download_{group_id}"),
        InlineKeyboardButton(text="📤 Excel yuklash", callback_data=f"grp_excel_upload_{group_id}")
    )
...
async def grp_delete_execute(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="🔙 Guruhlar ro'yxati", callback_data="admin_active_groups"))
        await callback.message.edit_text(f"✅ *{grp_name}* guruhi o'chirildi!", reply_markup=builder.as_markup(), parse_mode="Markdown")
    except Exception as e:
        await callback.answer(f"❌ Xatolik: {e}", show_alert=True)
    await callback.answer()
...
async def grp_delete_confirm(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"⚠️ *{grp['group_name']}* guruhini o'chirishni tasdiqlaysizmi?\n\nBarcha o'quvchilar ham o'chib ketadi!",
        reply_markup=builder.as_markup(), parse_mode="Markdown"
    )
    await callback.answer()

# ============================================================
# EXCEL ORQALI O'QUVCHILARNI BOSHQARISH
# ============================================================
...
async def grp_excel_download(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        ws.cell(row=note_row, column=1,
            value="ESLATMA: Yangi oquvchi - yangi qator. Ochirish - qatorni turing. Tahrirlash - togrilang. Keyin faylni qaytarib yuboring.")
        ...
        await callback.message.answer_document(
            types.BufferedInputFile(buf.read(), filename=filename),
            caption=(
                f"📥 *{grp['group_name']}* — o'quvchilar ro'yxati\n\n"
                f"✏️ Faylni tahrirlang:\n"
                f"• Yangi qator = yangi o'quvchi\n"
                f"• Qatorni o'chiring = o'quvchi o'chadi\n"
                f"• To'g'rilang = yangilanadi\n\n"
...
@dp.callback_query(F.data.startswith("grp_excel_upload_"))
async def grp_excel_upload_start(callback: types.CallbackQuery, state: FSMContext):
    if not check_admin(callback.message.chat.id):
        await callback.answer("Ruxsat yo'q!")
        return
    group_id = int(callback.data.replace("grp_excel_upload_", ""))
    ...
    await callback.message.answer(
        ...
        "⚠️ Fayl *to'liq ro'yxat* sifatida qabul qilinadi — "
        "fayldagi o'quvchilar saqlanadi, yo'q qatorlar o'chiriladi.",
        reply_markup=builder.as_markup(), parse_mode="Markdown"
    )
    await callback.answer()
...
async def grp_excel_process(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        result_text = (
            f"✅ *{grp['group_name']}* guruhi yangilandi!\n\n"
            f"👨‍🎓 Jami {len(new_students)} ta o'quvchi saqlandi:\n"
        )
        for idx, std in enumerate(new_students, 1):
            result_text += f"{idx}. {std['name']} — {std['phone']}\n"
...
async def std_submit_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_document(
        ADMIN_GROUP_ID,
        types.BufferedInputFile(buf.read(), filename=f"Davomat_{group_name}_{current_date}.xlsx"),
        caption=(
            f"🧑‍🎓 *Guruh Davomati*\n"
            f"📦 Guruh: {group_name}\n"
            f"👤 O'qituvchi: {teacher_name}\n"
