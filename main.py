...
async def admin_active_groups(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        builder = InlineKeyboardBuilder()
        for grp in all_grps:
            teacher_name = user_names.get(grp['teacher_id'], "Noma'lum o'qituvchi")
...
async def grp_view_detail(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        text = (
            ...
            f"📅 Kunlar: {days_str}\n\n"
            f"👨‍🎓 *O'quvchilar ro'yxati ({len(students)} ta):*\n"
        )
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(
            text="✏️ O'quvchilarni tahrirlash",
            callback_data=f"grp_edit_{group_id}"
        ))
        builder.row(
        ...
        builder.row(InlineKeyboardButton(
            ...
            callback_data=f"grp_delete_{group_id}"
        ))
...
async def grp_edit_students(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(
            "Tahrirlash uchun o'quvchini tanlang:",
            reply_markup=builder.as_markup()
        )
        await callback.answer()
    except Exception as e:
...
async def grp_std_edit_options(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(
            text="✏️ Ismini o'zgartirish",
            callback_data=f"grp_std_rename_{std_id}_{group_id}"
        ))
        builder.row(InlineKeyboardButton(
            text="📱 Raqamini o'zgartirish",
            callback_data=f"grp_std_rephone_{std_id}_{group_id}"
        ))
        builder.row(InlineKeyboardButton(
            text="🗑 O'quvchini o'chirish",
            callback_data=f"grp_std_del_{std_id}_{group_id}"
        ))
...
async def grp_std_save_name(message: types.Message, state: FSMContext):
    ...
    if data.get('add_std_step') == 'name':
        ...
        await state.set_state(EditGroupStudents.entering_new_phone)
        await message.answer(f"📱 *{new_name}* ning telefon raqamini kiriting:")
        return
    
    # Mavjud o'quvchi ismini o'zgartirish
    std_id = data['edit_std_id']
...
async def grp_std_save_phone(message: types.Message, state: FSMContext):
    ...
    # Yangi o'quvchi qo'shish - telefon
    if data.get('add_std_step') == 'phone':
        group_id = data['add_std_group_id']
        new_name = data['add_std_name']
        try:
            async with db.pool.acquire() as conn:
                await conn.execute(
    ...
    # Mavjud o'quvchi raqamini o'zgartirish
    std_id = data['edit_std_id']
    group_id = data['edit_group_id']
    try:
        async with db.pool.acquire() as conn:
...
async def grp_delete_execute(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if group_id in groups:
            del groups[group_id]
        if group_id in group_students:
            del group_students[group_id]

        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="🔙 Guruhlar ro'yxati", callback_data="admin_active_groups"))
        await callback.message.edit_text(
            f"✅ *{grp_name}* guruhi o'chirildi!",
            reply_markup=builder.as_markup(),
            parse_mode="Markdown"
        )
    except Exception as e:
    ...
    try:
        ...
        async with db.pool.acquire() as conn:
            ...
            students = await conn.fetch(
                "SELECT * FROM group_students WHERE group_id = $1 ORDER BY id", group_id
            )
        ...
        # Ustun sarlavhalari
        headers = ['№', 'Ism Familiya', 'Telefon raqami']
        ...
        ws.cell(row=note_row, column=1,
                value="⚠️ ESLATMA: Yangi o'quvchi qo'shish uchun yangi qator qo'shing. "
                      "O'chirish uchun qatorni o'chiring. Keyin faylni botga yuboring.")
        ...
        # ID qatori (yashirin, bot uchun)
        ws.cell(row=3, column=1, value="group_id")
        ws.cell(row=3, column=2, value=group_id)
        ...
        await callback.message.answer_document(
            types.BufferedInputFile(buf.read(), filename=filename),
            caption=(
                f"📥 *{grp['group_name']}* guruhining o'quvchilar ro'yxati\n\n"
                f"✏️ Faylni tahrirlang:\n"
                f"• Yangi o'quvchi — yangi qator qo'shing\n"
                f"• O'chirish — qatorni o'chiring\n"
                f"• Tahrirlash — to'g'rilang\n\n"
                f"Keyin faylni *shu guruhga* qaytarib yuboring (📤 Excel yuklash tugmasi)."
            ),
            parse_mode="Markdown"
        )
        await callback.answer()
...
async def grp_excel_upload_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "⚠️ Diqqat: Yuborilgan fayl *to'liq ro'yxat* sifatida qabul qilinadi — "
        "fayldagi o'quvchilar saqlanadi, yo'qoladigan qatorlar o'chiriladi.",
        reply_markup=builder.as_markup(),
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def grp_excel_process(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # O'quvchilarni o'qish (5-qatordan boshlab, 4-qator sarlavha)
        new_students = []
        for row in ws.iter_rows(min_row=5, values_only=True):
            num, name, phone = (list(row) + [None, None, None])[:3]
        ...
        # Bazani yangilash — eski o'quvchilarni o'chirib, yangisini yozish
        async with db.pool.acquire() as conn:
            grp = await conn.fetchrow("SELECT group_name FROM groups WHERE id = $1", group_id)
        ...
        builder.row(InlineKeyboardButton(
            ...
            callback_data=f"grp_view_{group_id}"
        ))
